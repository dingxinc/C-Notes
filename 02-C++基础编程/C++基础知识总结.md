...
5. `this` 并不是一个常规变量，而是个[[右值引用及移动构造 | 右值]]，所以不能取得 `this` 的地址（不能 `&this`）。
...
一条 `using 声明` 语句一次只引入命名空间的一个成员。它使得我们可以清楚知道程序中所引用的到底是哪个名字。如： [[using 的使用]]
...
在 C++11 中，派生类能够重用其直接基类定义的构造函数。[[委托构造和继承构造]]
...
[[自动类型推导#2. decltype | decltype]] 关键字用于检查实体的声明类型或表达式的类型及值分类。语法：
...
[分治算法详细讲解（含经典例题分析）_二分法不独立-CSDN博客](https://blog.csdn.net/qq_44398094/article/details/109920912?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522169586397516800227422866%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=169586397516800227422866&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-1-109920912-null-null.142^v94^insert_down28v1&utm_term=%E5%88%86%E6%B2%BB%E7%AE%97%E6%B3%95&spm=1018.2226.3001.4187)

[分 治 算 法_分治算法_代码骑士的博客-CSDN博客](https://blog.csdn.net/qq_51701007/article/details/123663352?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522169586397516800227422866%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=169586397516800227422866&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_click~default-2-123663352-null-null.142^v94^insert_down28v1&utm_term=%E5%88%86%E6%B2%BB%E7%AE%97%E6%B3%95&spm=1018.2226.3001.4187)
...
* 保留区：保留区并不是一个单一的内存区域，而是对内存中受到保护而禁止访问的内存区域的总称，如通常 C 语言将无效指针赋值为 0（NULL），因此 0 地址正常情况下不可能有效的访问数据
